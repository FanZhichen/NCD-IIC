...
from utils.transforms import DiscoverTargetTransform  # Only for ImageNet
...
class PretrainCIFARDataModule(pl.LightningDataModule):
    def __init__(self, args):
        ...
        self.dataset_class = getattr(torchvision.datasets, args.dataset)  # 返回对象的属性值，即获得torchvision中所需的数据集类，用于实例化
    ...
    def setup(self, stage=None):
        ...
        train_indices_lab = np.where(np.isin(np.array(self.train_dataset.targets), labeled_classes))[0]  # 获得所有标签落在所设区间的图像的索引
...
class DiscoverImageNetDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        train_data_dir = os.path.join(self.data_dir+'/IMAGENET2012', "train")
        val_data_dir = os.path.join(self.data_dir+'/IMAGENET2012', "val")
