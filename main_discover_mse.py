...
parser.add_argument("--factor_mse", default=0.01, type=float, help="factor for MSE")
...
class Discoverer(pl.LightningModule):
    ...
    def mse(self, logits, mask_lab):
        prob_lab = torch.exp(F.log_softmax(logits[:, :, mask_lab, :self.hparams.num_labeled_classes] / self.hparams.temperature, dim=-1))
        prob_unlab = torch.exp(F.log_softmax(logits[:, :, ~mask_lab, self.hparams.num_labeled_classes:] / self.hparams.temperature, dim=-1))

        mse_loss = 0
        for view_i in range(self.hparams.num_crops):
            prob_lab_view_i = prob_lab[view_i]
            prob_unlab_view_i = prob_unlab[view_i]
            for view_j in range(view_i + 1, self.hparams.num_crops):
                prob_lab_view_j = prob_lab[view_j]
                prob_unlab_view_j = prob_unlab[view_j]
                mse_loss_lab = F.mse_loss(prob_lab_view_i, prob_lab_view_j, reduction='mean')
                mse_loss_unlab = F.mse_loss(prob_unlab_view_i, prob_unlab_view_j, reduction='mean')
                mse_loss += (mse_loss_lab + mse_loss_unlab)
        return mse_loss / comb(self.hparams.num_crops, 2, exact=True)
    ...
    def training_step(self, batch, _):
        ...
        # 计算MSE
        mse_loss = self.mse(logits, mask_lab)  # MSE
        ...
        loss = (loss_cluster + loss_overcluster) / 2 + mse_loss * self.hparams.factor_mse
